...
取得するのは通常のチャンネル履歴の本文・投稿者・時刻・添付ファイル名です。添付ファイルの本体はダウンロードしません。絵文字リアクション、投稿の編集履歴、削除済みメッセージ、埋め込み表示などは保存・再現しません。[ビューア]({{ '/viewer/' | relative_url }})は保存された本文を簡易表示するもので、Slack / Discord の画面を完全に再現するものではありません。
